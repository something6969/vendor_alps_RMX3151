...
SDCARD_LOG_BASE_PATH=${BASE_PATH}/Android/data/com.oplus.logkit/files/Log
...
function logObserver() {
    ...
    if [[ -f "${LOG_CONFIG_FILE}" ]]; then
        ...
        do
            if [[ "" != "${ITEM_CONFIG}" ]];then
                #traceTransferState "transfer log config: ${ITEM_CONFIG}"
                ...
                if [[ -d ${SOURCE_PATH} ]];then
                    ...
                    traceTransferState "PATH: ${SOURCE_PATH}, No such file or directory"
                fi
            fi
...
function traceTransferState() {
    ...
    content=$1
    echo "${CURTIME_FORMAT} ${content} " >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
...
function clearDebuggingLog(){
    ...
    traceTransferState "clear debugging/debuglogger log done"
...
function cleanLog(){
    ...
    traceTransferState "clean log done"
    setprop sys.clear.finished 1
}
