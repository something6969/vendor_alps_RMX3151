        ...
        while [ $exception_count -lt $exception_max ] ;do
            ...
            if [ x"${inputreader_stack_status}" == x"futex_wait_queue_me" ]; then
                ...
                if [ x"${exception_count}" = x"${exception_max}" ]; then
                    ...
                    setprop sys.oplus.futexwaitblocked "`date`"
...
function transferSystemAppLog(){
    ...
    YOLI_LOG2=/sdcard/Android/data/com.coloros.yoli/files/yoliVideo/xlog
    checkSmallSizeAndCopy "${YOLI_LOG2}" "os/com.coloros.yoli"
...
function transferDataHprof(){
    ...
    TARGET_DATA_HPROF_LOG=${newpath}/log
